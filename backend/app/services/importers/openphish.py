"""
openphish.py - OpenPhish feed importer
"""
...
class OpenPhishImporter(BaseImporter):
    """Ingests OpenPhish plain-text feed list"""

    URL = "https://openphish.com/feed.txt"
    REDIS_KEY = "feeds:openphish"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(self.URL, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    if resp.status != 200:
                        logger.warning(f"OpenPhish importer returned status code {resp.status}")
            ...
            logger.error(f"OpenPhish importer error: {e}")
            db.rollback()
